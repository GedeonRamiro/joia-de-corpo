    ...
    return (
        <>
            <div className='drawer h-screen'>
                ...
                <div className='flex justify-between flex-col drawer-content'>
                    <div>
                                    ...
                                    <div className='items-stretch  hidden sm:flex justify-center mt-2'>
                                        ...
                                        {menuArray.map((menu, index) => (
                                            <Link key={index} href={menu.href}>
                                                <a
                                                    className={`btn btn-ghost text-black btn-sm font-medium rounded-btn no-animation mx-2 ${
                                                        menu.href === asPath ? 'bg-pink-400' : ''
                                                    }`}
                                                >
                                                    {menu.name}
                                                </a>
                ...
                <div className='drawer-side'>
                    ...
                    <ul className='p-4 overflow-y-auto menu w-80 bg-pink-50'>
                        {menuArray.map((menu, index) => (
                            <Link key={index} href={menu.href}>
                                <a
                                    className={`btn btn-ghost btn-sm font-medium rounded-btn no-animation ${
                                        menu.href === asPath ? 'bg-pink-500' : ''
                                    }`}
                                >
                                    {menu.name}
                                </a>
