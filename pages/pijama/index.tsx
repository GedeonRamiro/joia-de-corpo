...
import bannerPijama from '../../public/images/banner-categoria.png';
...
const Pijama = ({ products }: ContentPros) => {
    return (
        <>
            <Head>
            ...
            <Header>
                <div className='container mx-auto'>
                    ...
                    <h2 className='text-center text-2xl sm:text-3xl font-semibold mt-10'>PIJAMA</h2>
                    <p className='text-center sm:text-base text-sm sm:mx-0 mx-2'>
                        Veja aqui toda linha de produtos dedicados as mulheres que procuram além do
                        charme, tendências e qualidade que podemos oferecer.
                    </p>
                    <div className='grid gap-2 sm:gap-6 my-10 md:my-20 grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
                        {products &&
                            products.map((product) => (
                                <div
                                    className='card text-center sm:shadow-2xl sm:mx-2'
                                    key={product.id}
                                >
                                    ...
                                    <div className='flex-col flex justify-between'>
                                        ...
                                        <div className='justify-center card-actions mb-4'>
                                            <Link href={`/pijama/${product.slug}`}>
                                                <button className='sm:btn-md btn btn-sm bg-pink-400 hover:bg-pink-500 border-none'>
                                                    Comprar
                                                </button>
                                            </Link>
                                        </div>
...
export const getServerSideProps: GetServerSideProps = async () => {
    ...
    const products = resultProducts.results
        .map((data) => ({
            id: data.id,
            slug: data.uid,
        ...
        .filter((data) => data.category === 'Pijama');
